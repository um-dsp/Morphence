...
def resnet18(in_channel=3, **kwargs):
    return ResNet(BasicBlock, [2, 2, 2, 2], in_channel= in_channel, **kwargs)


def resnet34(in_channel=3,**kwargs):
    return ResNet(BasicBlock, [3, 4, 6, 3], in_channel= in_channel, **kwargs)


def resnet50(in_channel=3,**kwargs):
    return ResNet(Bottleneck, [3, 4, 6, 3], in_channel= in_channel, **kwargs)


def resnet101(in_channel=3,**kwargs):
    return ResNet(Bottleneck, [3, 4, 23, 3], in_channel= in_channel, **kwargs)
...
class SSLResNet(nn.Module):
    def __init__(self, arch="resnet50", in_channel=3, out_dim=128, **kwargs):
        super(SSLResNet, self).__init__()
        #print(**kwargs)
        m, fdim = model_dict[arch]
        self.encoder = m(in_channel)
